  ...
  <div>
    ...
    <PeaceTable ref="table"
      ...
      <el-table-column min-width="100px"
                       label="模板内容"
                       prop="commonContent"></el-table-column>
    ...
    <PeaceDialog width="480px"
                 ...
                 :title="dialog.title">
      <AddCommentTemplate v-if="dialog.visible"
                          v-on:refresh="fetch"
                          v-on:close="dialog.visible = false"></AddCommentTemplate>
    </PeaceDialog>

    <PeaceDialog width="480px"
                 v-bind:visible.sync="editDialog.visible"
                 :title="editDialog.title">
      <AddCommentTemplate v-if="editDialog.visible"
                          v-bind:data="editDialog.data"
                          v-on:refresh="fetch"
                          v-on:close="editDialog.visible = false"></AddCommentTemplate>
    </PeaceDialog>
  ...
  methods: {
    ...
    deleted(row) {
      this.$confirm('确定删除模板？', '提示', {
        ...
        Service.delCommentTemplates(params).then(() => {
    ...
    fetch() {
      ...
      const fetch = Service.getCommentTemplatesList
      const filter = (res) => {
