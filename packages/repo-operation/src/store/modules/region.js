export default {
  ...
  actions: {
    getRegion({ commit }) {
      if (Peace.cache.localStorage.get('region')) {
        // 如果存在使用缓存的 Region
        const region = JSON.parse(Peace.cache.localStorage.get('region'))
        ...
        return Peace.http.get(servicePath).then((res) => {
          ...
          Peace.cache.localStorage.set('region', JSON.stringify(region))
          commit('setRegion', region)
          return region
        })
      }
