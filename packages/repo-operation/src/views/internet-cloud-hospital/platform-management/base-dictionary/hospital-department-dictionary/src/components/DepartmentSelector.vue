  ...
  methods: {
    ...
    getList(value, pname) {
      ...
      if (pname) {
        ...
        return Service.getLevel2DepartmentList({ platdeptName: pname, platdeptLevel2Name: deptName, p: pageNumber, size: pageSize }).then((res) => {
      ...
      // 查询一级科室列表
      return Service.getLevel1DepartmentList({ platDeptName: '', p: pageNumber, size: pageSize }).then((res) => {
        this.total = res.data.total
        return Promise.resolve(res.data.list)
      })
    }
