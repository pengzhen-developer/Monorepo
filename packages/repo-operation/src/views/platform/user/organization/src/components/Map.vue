    ...
    <div class="control">
      <el-form inline
        ...
        <el-form-item label="所属辖区"
          ...
          <el-button :disabled="!isSelected"
                     @click="saved"
                     type="primary">选定地址</el-button>
        </el-form-item>
        <el-form-item label="详细地址"
                      class="is-required">
          <el-autocomplete :fetch-suggestions="keywordSearch"
                           ...
                           value-key="_address"></el-autocomplete>
        </el-form-item>
      </el-form>
      <div class="info-row two-cols mb-lg pr-xs pl-sm">
        ...
        <div class="info-row-content pr-lg">
          <el-input disabled
                    readonly
                    v-model="selectedItem.location.lat"></el-input>
        </div>
        <div class="info-row-label">
          <span>纬度</span>
        </div>
        <div class="info-row-content pr-lg">
          <el-input disabled
                    readonly
                    v-model="selectedItem.location.lng"></el-input>
        </div>
      </div>
    </div>
...
const filterRegion = (str) => {
  ...
  return str.replace(str.match(reg).join(''), '').replace(regTrim, '').replace('中国', '')
}
  ...
  methods: {
    ...
    bykeywordSearchCallBack(result) {
      ...
      if (isCitySearch) {
        ...
        pois.map((element) => {
          ...
          element._address = address ? `${address} (${element.name})` : element.name
        })
        this.callback(pois)
        setTimeout(() => {
          this.callback = null
    ...
    addressEcho(item) {
      ...
      if (this.isClick) {
        ...
        this.selectedItem.address = address ? `${address} (${target.name})` : target.name
      ...
      const _province = getIdByLabel(region, item.detail.addressComponents.province)
    ...
    setDefaults() {
      ...
      this.selectedItem.address = this.defaultDetail.address || ''
      this.selectedItem.location.lat = this.defaultDetail.latitude || ''
      this.selectedItem.location.lng = this.defaultDetail.longitude || ''
      if (this.defaultDetail.address) {
        this.isSelected = true
      }
      if (this.api.marker) {
