<template>
  <div>
    ...
    <div id="map-container" ref="TMap"></div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getMapScript () {
      if (!global.QMap) {
        global.QMap = {}
        global.QMap._preloader = new Promise((resolve, reject) => {
          ...
          script.src = `https://map.qq.com/api/js?v=2.exp&key=${KEY}&libraries=place&callback=_initQMap`
        })
    ...
    initMap() {
      if (!this.map) {
        this.map = new qq.maps.Map(this.$refs.TMap, {
