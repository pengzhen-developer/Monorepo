  ...
  <PeaceDialog title="操作日志"
    ...
    <el-timeline v-if="logList.length">
      <TimeLineItem type="primary"
                    ...
                    v-bind:key="index"
                    v-bind:timestamp="Peace.dayjs(log.updateTime).format('YYYY-MM-DD')"
                    v-bind:timestampSpan="Peace.dayjs(log.updateTime).format('HH:mm:ss')">
        <div style="color: #3099A6; font-weight:500">{{ log.operatorStates}}审方设置</div>
...
export default {
  ...
  methods: {
    fetch() {
      Service.getLog({ hospitalCode: this.data.SourceHospitalCode }).then((res) => {
