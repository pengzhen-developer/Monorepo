<template>
  <PeaceDialog title="特殊重复用药规则操作日志"
               v-bind="$attrs"
    ...
    <el-timeline v-if="logList.length">
      <TimeLineItem type="primary"
                    ...
                    v-bind:key="index"
                    v-bind:timestamp="Peace.dayjs(log.operationTime).format('YYYY-MM-DD')"
                    v-bind:timestampSpan="Peace.dayjs(log.operationTime).format('HH:mm:ss')">
        <div style="color: #3099A6; font-weight:500">{{ log.operationStatus| filterDictionary(statusList,'') }}</div>
        <div style="color: rgba(51, 51, 51, 1);">{{ log.operationName }}</div>
      </TimeLineItem>
    </el-timeline>
...
export default {
  ...
  async created() {
    this.statusList = await Peace.identity.dictionary.getList('operation_status')
  },
  mounted() {
    this.$nextTick().then(() => {
      this.fetch()
  ...
  methods: {
    fetch() {
      Service.getLog({ ruleId: this.data.ruleId, ruleType: 'unionduplicatetherapy' }).then((res) => {
