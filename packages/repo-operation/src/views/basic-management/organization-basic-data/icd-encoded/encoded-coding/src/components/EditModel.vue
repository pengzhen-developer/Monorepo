...
<template>
  <div v-loading="loading">
    <div class="flex justify-between "
      ...
      <div class="flex column items-center">
        ...
        <div class="info">
          <div class="info-item flex">
            ...
            <div class="info-item-value">{{innerInfo.icd10Code}}</div>
          </div>
          <div class="info-item flex">
            <div class="info-item-label">附加编码</div>
            <div class="info-item-value">{{innerInfo.extCode}}</div>
          </div>
          <div class="info-item flex">
      ...
      <div class="text-primary flex column items-center justify-center q-pt-12 q-mx-16"
      ...
      <div class="flex column items-center">
        ...
        <div class="info">
          <div class="info-item flex">
            <div class="info-item-label">主要编码</div>
            <div class="info-item-value">{{innerInfo.platformDiagnosisCode}}</div>
          </div>
          <div class="info-item flex">
            <div class="info-item-label">附加编码</div>
            <div class="info-item-value">{{innerInfo.platformDiagnosisExtCode}}</div>
          </div>
          <div class="info-item flex">
            <div class="info-item-label">疾病名称</div>
            <div class="info-item-value">{{innerInfo.platformDiagnosisName}}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    //保存配码
    submit() {
      if (!this.innerInfo.platformDiagnosisCode) {
        Peace.util.warning('请选择平台编码信息')
      }
    ...
    matchCodeOperation(type) {
      ...
      const params = {
        ...
        orgCode: this.innerInfo.orgCode,
        //平台字典
        platformDiagnosisCode: this.innerInfo.platformDiagnosisCode,
        platformDiagnosisName: this.innerInfo.platformDiagnosisName,
        platformDiagnosisExtCode: this.innerInfo.platformDiagnosisExtCode
      }
      Service.matchCodeOperation(params)
        .then((res) => {
          ...
          if (type === 'Save') {
            ...
            //重置配码，无法配码 清除平台字典信息
            this.radioId = ''
            this.innerInfo.platformDiagnosisCode = ''
            this.innerInfo.platformDiagnosisName = ''
            this.innerInfo.platformDiagnosisExtCode = ''
          }
          this.clear()
          this.refresh()
        })
    ...
    fetch() {
      ...
      const fetch = Service.matchPlatformDiagnosis
      this.$refs.table
    ...
    selectItems(row) {
      this.innerInfo.platformDiagnosisCode = row.icd10Code
      this.innerInfo.platformDiagnosisName = row.name
      this.innerInfo.platformDiagnosisExtCode = row.extCode
    }
  }
}
</script>
