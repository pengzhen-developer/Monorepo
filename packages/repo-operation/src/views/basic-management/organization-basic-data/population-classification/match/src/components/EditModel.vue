...
export default {
  ...
  mounted() {
    this.$nextTick().then(() => {
      ...
      if (this.organInfo.mapperStatus === 'success') {
        this.platInfo = { name: this.organInfo.platformClassName, code: this.organInfo.platformClassCode }
      }
      this.fetch()
    })
  },
  methods: {
    fetch() {
      ...
      const fetch = Service.matchPlatformHumanClass
      this.$refs.table
