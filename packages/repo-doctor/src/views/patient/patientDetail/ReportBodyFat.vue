...
<template>
  <div class="report">
    <div class="report-header">
      ...
      <el-button-group>
        <el-button :type="view.active === source.active['体脂数据'] ? 'primary' : '' " @click="changeActive('体脂数据')">体脂数据</el-button>
        <el-button :type="view.active === source.active['历史数据'] ? 'primary' : '' " @click="changeActive('历史数据')">历史数据</el-button>
      </el-button-group>
    </div>

    <div class="report-recent" v-show="view.active === source.active['体脂数据']">
      <el-row :gutter="20">
        <el-col :span="12">
          <el-card shadow="never">
            ...
            <div class="report-recent-row">
              <div class="report-recent-col">
                <label>体脂率：</label>
                <span>{{ view.model.bfr }}%</span>
              </div>

              <div class="report-recent-col">
                <label>测量时间：</label>
                <span>{{ view.model.createTime }}</span>
              </div>
            </div>
            <div class="report-recent-row">
              <label>体重：</label>
    ...
    <div class="report-history" v-show="view.active === source.active['历史数据']">
      <peace-table pagination ref="tableHistory" v-show="this.$route.params.type === $peace.type.HEALTH.TYPE.体脂">
        <peace-table-column label="体脂率（%）" prop="bfr"></peace-table-column>
        <peace-table-column label="分析结果" prop="result"></peace-table-column>
        <peace-table-column label="检测时间" prop="createTime"></peace-table-column>
      ...
      peace.service.health.getOscillogram(params).then(res => {
        ...
        options.tooltip.formatter = function(params) {
          return `<span style="width: 5rem; text-align: right; display: inline-block; color:rgba(153,153,153,1); margin: 0 0 10px 0;">体脂率：</span> 
                  ...
                  <br/>
                  <span style="width: 5rem; text-align: right; display: inline-block; color:rgba(153,153,153,1); margin: 0 0 10px 0;">体重：</span> 
                  <span style="color: rgba(51,51,51,1);">${params[0].data.weight}kg</span>
                  <br/>
                  <span style="width: 5rem; text-align: right; display: inline-block; color:rgba(153,153,153,1); margin: 0 0 10px 0;">分析结果：</span> 
                  ...
                  <span style="color: rgba(51,51,51,1);">${params[0].data.createTime}</span>
                   `
        }
        ...
        this.view.model = res.data.upInfo.bodyFat
      })
    },
