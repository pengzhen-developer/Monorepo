<template>
  <div class="consultation-session-message-input">
    <div class="control">
      ...
      <el-button @click="sendInvited" type="text"
      ...
      <el-button @click="snedConsultSuggest" type="text"
        v-show="$store.getters['consultation/consultInfo'].receiveDoctor.find(item => item.doctorId === $store.state.user.userInfo.list.docInfo.doctor_id) && 
                $peace.consultationComponent.getIntervalStatus(this.$store.state.consultation.session) === $peace.type.CONSULTATION.CONSULTATION_STATUS_EXTEND.会诊中">
    ...
    <peace-dialog :visible.sync="invitedDialog.visible" class="invited-dialog" title="邀请协同医生">
      <div>
        <el-input placeholder="请输入医生姓名、职称、科室" style="width: calc(100% - 120px); margin-right: 20px;"
      ...
      <peace-table :layout="'total,  -> , prev, pager, next, slot'" :pageSize="5" pagination
        ref="table" v-if="invitedDialog.visible">
      ...
      <template v-if="invitedDialog.chooseList.length">
        ...
        <div style="border:1px solid rgba(204,204,204,1); min-height: 110px;">
          <el-tag :key="item.doctorId" @close="closeInvitedChange(item.doctorId)" closable
            style="margin: 10px; height: 32px; padding: 5px 15px; " type
...
export default {
  ...
  methods: {
    ...
    sendVideo() {
      $peace.videoComponent.call(this.$store.state.consultation.session)
    },
