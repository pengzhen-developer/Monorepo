<template>
  <div class="pres">
    <div class="tips"
         v-if="data.total > 1">
      <span>共{{ data.total }}张，当前第{{ prescribeIndex }}张（{{ prescribeIndex }}/{{ data.total }}）</span>
      <div @click="goToNext()"
           class="next">
        <span>下一张</span>
        <i class="arrow"></i>
...
export default {
  ...
  watch: {
    prescribeIndex: {
      handler() {
        const params = {
          prescriptionId: this.data.list[this.prescribeIndex - 1].id
        }
  ...
  methods: {
    ...
    goToNext() {
      if (this.prescribeIndex == this.data.total) {
