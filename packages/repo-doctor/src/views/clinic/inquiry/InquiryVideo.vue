<template>
  <div
    class="inquiry-video"
    v-if="$store.state.inquiry.session && $store.state.inquiry.session.id"
  >
    <el-dialog
    ...
    >
      <div class="videoContaier" id="localContainer">
        <div style="position: absolute; text-align: center;">
          <p style="font-size: 14px;">
            {{ $store.state.user.userInfo.list.docInfo.name }}
          </p>
      ...
      <div class="videoContaier" id="remoteContainer">
        <div style="position: absolute; text-align: center;">
          <p style="font-size: 14px;">
            {{ $store.state.inquiry.session.content.patientInfo.familyName }}
          </p>
          <p style="font-size: 14px;">等待加入中</p>
          <i
            class="el-icon-loading"
...
export default {
    ...
    beCalledTime() {
      ...
      if (this.beCalledTime) {
        this.beCalledTimeInterval = setInterval(() => {
          this.inquiry.title = peace.util.formatDuration(
            this.beCalledTime,
            new Date()
          );
        }, 500);
  ...
  methods: {
    ...
    onCallAccepted(callAcceptedObject) {
      ...
      const params = {
        inquiryNo: this.$store.state.inquiry.session.content.inquiryInfo
          .inquiryNo,
        action: "start"
      };
...
>>>>>>> feature-新建随访方案
    ...
    onHangup(hangupObject) {
      ...
      if (
        !this.beCalledInfo ||
        this.beCalledInfo.channelId === hangupObject.channelId
      ) {
    ...
    onCallerAckSync(callerAckObject) {
      ...
      if (
        this.beCalledInfo &&
        callerAckObject.channelId === this.beCalledInfo.channelId
      ) {
        // 清空状态
        this.clearState();
