  ...
  <div>
    ...
    <div class="perscription">
      ...
      <peace-table :data="prescription.source.list" ref="table">
        <peace-table-column label="药品名称" prop="drug_name" show-overflow-tooltip>
        </peace-table-column>
    ...
    <peace-dialog :visible.sync="drug.visible" @close="closeDialog" title="新增药品">
      <el-form :model="drug.model" :rules="drug.rules" :validate-on-rule-change="false"
        label-position="right" label-width="80px" ref="form">
        <el-form-item label="药品名称" prop="drugid">
          <el-autocomplete :fetch-suggestions="querySearchAsync" @select="handleSelectDrug"
            placeholder popper-class="drug-autocomplete" style="width: 100%;"
          ...
          <el-col :span="8">
            <el-form-item label="给药途径" prop="dic_usage_id">
              <el-select @change="dicUsageChange" placeholder v-model="drug.model.dic_usage_id">
                <el-option :key="item.id" :label="item.drugway_name" :value="item.id"
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="单次剂量" prop="consump" style="white-space: nowrap;">
              <el-input placeholder style="width: 60px; margin-right: 10px;"
          ...
          <el-col :span="16">
            <el-form-item label="用药频次" prop="dic_frequency_id">
              <el-select @change="dicFrequencyChange" placeholder style="width: 100%;"
                v-model="drug.model.dic_frequency_id">
                <el-option :key="item.id" :label="item.drugtimes_name" :value="item.id"
        ...
        <el-row>
          <el-button @click="addDrug" round style="float: right;" type="primary"
            v-show="drug.action === drug.ACTION_TYPE.INSERT">添加到处方</el-button>
          <el-button @click="saveDrug" round style="float: right;" type="primary"
      ...
      <div style="text-align: center; margin: 30px 0 0 0;">
        ...
        <el-button @click="saveDrugToPrescription" type="primary" :disabled="drug.disabled">保存
        </el-button>
      </div>
    </peace-dialog>
  </div>
