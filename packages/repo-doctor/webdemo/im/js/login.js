	...
	*/
	...
	validate: function() {	// 登录验证
        ...
        if (account.length === 0) {
            ...
            that.$loginBtn.html('登录中...').attr('disabled', 'disabled');
            that.requestLogin.call(that, account);
            that.$loginBtn.html('登录').removeAttr('disabled');
        }
        that.$errorMsg.html(errorMsg).removeClass('hide');  // 显示错误信息
        return false;
	},
	//登录（实际上是把accid，token带到下个页面连SDK在做鉴权）
	requestLogin:  function(account) {
        var that = this;
        $.ajax({
            type: 'POST',
            url: "https://devapi.hp.aijiayi.com/doctorapi/demo/demo",
            data: {
                username: account,
            },
            dataType: "json",
            ...
            console.log('res', res)
            setCookie('uid',res.data.accid);
            setCookie('sdktoken',res.data.token);
