<template>
  <div class="container">
    <template v-if="isEdit">
      ...
      <div class="card" v-for="(cardItem, index) in cardList" :key="'item' + index" @click="goToDetail()">
        <div class="qrcode" id="qrcode" :style="{backgroundImage:'url(data:image/png;base64,'+ cardItem.base64 + ')'}"></div>
...
<!--      <div class="card no-card"></div>-->
      <div class="form form-for-family">
        ...
        <van-field @click="showPopupNations" label="民族" placeholder="请输入" readonly required right-icon="arrow"  v-model="model.nationName"/>
...
<!--        <van-field :value="model.foodAllergy" @click="showFoodAllergy= true" clickable label="食物/接触物过敏" placeholder="请选择" readonly right-icon="arrow" />-->
      ...
      <peace-dialog :title="gDialog.title"
          ...
          <GuardianList :data="gDialog.data" @setGardianInfo="setGardianInfo"/>
      </peace-dialog>
      </div>
      <div class="bottom">
    ...
    <!-- 民族 -->
    <van-popup position="bottom" v-model="showNations">
      <van-picker :columns="nations" @cancel="closeAllPopup" @confirm="setNations" show-toolbar/>
    </van-popup>
