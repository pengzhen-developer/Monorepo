<template>
    <div v-if="data.name">
        ...
        <div class="box-bg"   :style="data.diseaseLists.length > 5 ? (page.nextClass ? 'padding-bottom: 35px' : 'max-height:65px;padding-bottom: 35px' ): ''">
            <div v-for="(item, index) in data.diseaseLists"
                 :key="index"
                  :class="['label', checkValue == item.code ? 'blue' : '']" @click="goDianodeHomePage(item)" data-code="item.code" data-value="item.name">
                {{item.name}}
            </div>
            <div class="more" @click="toggleNext" v-if="data.diseaseLists.length > 5">
    ...
    export default {
        ...
        methods: {
            getBodySymptomDetail() {
                peace.service.diagnose.groupDetail({
