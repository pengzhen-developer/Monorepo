  ...
  <div class="layout-route">
    <div class="card card-search q-mb-md">
      <el-form inline
        ...
        <el-form-item label="机构名称">
          <PeaceInput v-model.trim="model.instituteName"
    ...
    <div class="card">
      <PeaceTable pagination
        ...
        <PeaceTableColumn label="药品目录数量"
                          prop="drugCount"
                          min-width="120"></PeaceTableColumn>
        <PeaceTableColumn label="药品启用数量"
                          prop="drugOnCount"
                          min-width="120"></PeaceTableColumn>
        ...
        <PeaceTableColumn label="操作"
          ...
          <template slot-scope="scope">
            <PeaceButton type="text"
                         v-on:click="gotoDrugList(scope.row)">查看详情</PeaceButton>
          </template>
        </PeaceTableColumn>
      </PeaceTable>
    </div>
...
export default {
  ...
  methods: {
    fetch() {
      const fetch = Service.drugCountList
    ...
    gotoDrugList(row) {
      ...
      obState.mutations.setView('DrugList')
    }
  }
}
</script>
