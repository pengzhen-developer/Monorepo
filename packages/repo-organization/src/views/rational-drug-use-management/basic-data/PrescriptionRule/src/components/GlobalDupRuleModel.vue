<template>
  <div>
    <PeaceTable ref="table"
      ...
      <PeaceTableColumn label="成分/成分类别"
        ...
        <template slot-scope="scope">
          <div class="info-styles">{{scope.row.dupItemInfos}}</div>
        </template>
      </PeaceTableColumn>
      <PeaceTableColumn label="重复的成分/成分类别"
                        prop="dupContraryInfos"
                        min-width="200px">
        <template slot-scope="scope">
          <div class="info-styles">{{scope.row.dupContraryInfos}}</div>
        </template>
      </PeaceTableColumn>
    ...
    <peace-dialog :close-on-click-modal="false"
                  ...
                  append-to-body
                  width="576px">
      <GlobalDupRuleDetailModel v-bind:info="detailModelDialog.data" />
    </peace-dialog>
  </div>
</template>

<script>
import GlobalDupRuleDetailModel from './GlobalDupRuleDetailModel'
import Service from '../service/index'
export default {
  components: {
    GlobalDupRuleDetailModel
  },
  data() {
    return {
      source: {
  ...
  methods: {
    fetch() {
      const fetch = Service.getDupRuleList
