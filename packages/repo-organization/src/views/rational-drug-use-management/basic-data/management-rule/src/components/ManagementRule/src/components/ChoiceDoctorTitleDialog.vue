  ...
  <div>
    <PeaceDialog title="添加条件"
      ...
      <div v-show="multipleSelection.length > 0"
           class="q-mb-40">
        <span class="text-grey-333">已选职级：</span>
        <div class="q-mt-md">
      ...
      <peace-table ref="table"
        ...
        <peace-table-column label="职级编码"
                            min-width="120px"
                            prop="code">
        </peace-table-column>
        <peace-table-column label="职级名称"
                            min-width="120px"
                            prop="name">
          <template slot-scope="scope">
            {{scope.row.name||"——"}}
...
export default {
  ...
  methods: {
    fetch() {
      const fetch = Service.commonDictDoctorTitleList
      const params = {}
