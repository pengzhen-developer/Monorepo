<template>
  <div class="layout-route">
    <div class="card card-search q-mb-md">
      <el-form inline=""
        ...
        <el-form-item>
          ...
          <el-input v-model="model.toDoctorName"
        ...
        <el-form-item>
          ...
          <el-input v-model="model.toHospitalName"
                    placeholder="请输入"></el-input>
        </el-form-item>
        ...
        <PeaceTableColumn fixed="right"
          ...
          <template slot-scope="scope">
            <el-button @click="getConsultDetail(scope.row.consultNo)"
                       type="text"
                       v-if="scope.row.consultStatus === 1">审核</el-button>
            <el-button @click="getConsultDetail(scope.row.consultNo)"
                       type="text"
                       v-else>详情</el-button>
          </template>
    ...
    <PeaceDialog :visible.sync="dialogVisible"
                 ...
                 v-if="dialogVisible"
                 width="580px">
      <consult-launch-info :data="currentConsultation"
                           @close="close"
                           @update="getList"></consult-launch-info>
    </PeaceDialog>
  </div>
</template>
...
import ConsultLaunchInfo from './components/ConsultLaunchInfo'

export default {
  name: 'ConsultationLaunch',
  components: { ConsultLaunchInfo },
  data() {
    return {
      source: {
  ...
  methods: {
    getList() {
      const fetch = Service.getConsultLaunchList
