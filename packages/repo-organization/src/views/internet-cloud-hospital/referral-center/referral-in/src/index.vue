<template>
  <div class="layout-route">
    <div class="card card-search q-mb-md">
      <el-form inline=""
        ...
        <el-form-item>
          ...
          <el-input v-model="model.inDoctorName"
        ...
        <el-form-item>
          ...
          <el-input v-model="model.outHospitalName"
                    placeholder="请输入"></el-input>
        </el-form-item>
        ...
        <PeaceTableColumn fixed="right"
          ...
          <template slot-scope="scope">
            <el-button @click="getReferralDetail(scope.row.id, scope.row.referral_log_id)"
                       type="text"
                       v-if="scope.row.transfer_status === 3">审核</el-button>
            <el-button @click="getReferralDetail(scope.row.id, scope.row.referral_log_id)"
                       type="text"
                       v-else>详情</el-button>
          </template>
    ...
    <PeaceDialog :visible.sync="dialogVisible"
                 ...
                 width="580px"
                 append-to-body>
      <referral-in-info :id="currentId"
                        :info="currentReferral"
                        @close="closeDialog"
                        @update="getList"></referral-in-info>
    </PeaceDialog>
  </div>
</template>
...
import ReferralInInfo from './components/ReferralInInfo'

export default {
  name: 'ReferralOut',
  components: { ReferralInInfo },
  data() {
    return {
      source: {
  ...
  methods: {
    getList() {
      const fetch = Service.getReferralInList
